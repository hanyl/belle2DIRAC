...
function createDSTree( rootNodeName, editable )
{
	if( editable )
		gEditableMode = true;
	   ...
	   gSectionMenu = new Ext.menu.Menu({
		...
		gValuePanel = new Ext.FormPanel ({
			...
			buttons : [ { text: 'Submit',
	                  ...
	                  { text: 'Reset',
	                    handler: function(e) {
	                    		gValuePanel.getComponent( 'valueArea' ).setValue( gValuePanel.dsValue );
	                    	},
	                  },
	                  { text : 'Cancel',
	                    handler : function( e ) {
...
{
	node.setText( node.attributes.csName + " = " + node.attributes.dsValue );
	if( gEditableMode )
...
function inPlaceOptionValueChange( node, clickEvent )
{
	...
	node.setText( node.attributes.csName + " = <input id='"+editId+"' type='text' size='100' value='" + node.attributes.dsValue +"'/>" );
...
function serverSetOptionValue( ajaxResponse, reqArguments )
{
	...
	node.attributes.dsValue = reqArguments.params.value;
	configureLeafNode( node );
}
...
function cbMenuSetOptionValue( menuItem, clickEvent )
{
	...
	gValuePanel.dsValue = stringToList( node.attributes.dsValue ).join( "\n" );
...
function cbMenuSetCommentValue( menuItem, clickEvent )
{
	var node = menuItem.parentMenu.targetCtxNode;
	gValuePanel.csNode = node;
	gValuePanel.dsValue = commentToList( node.attributes.csComment ).join( "\n" );
	gValuePanel.csPath = getNodePath( node );
...
function serverCopyNode( ajaxResponse, reqArguments )
{
	...
	{
		newCfg.leaf = true;
		newCfg.dsValue = node.attributes.dsValue;
	}
	else
	{
		newCfg.text = newName;
...
function serverCreateSection( ajaxResponse, reqArguments )
{
	...
	var dsData = retVal.Value;
	var newCfg = { text : dsData.csName,
						csName : dsData.csName,
						csComment : dsData.csComment,
						leaf : false };
	var node = reqArguments.node;
	if( node.isLoaded() )
	{
